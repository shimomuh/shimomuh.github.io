...
タグ&nbsp;<span className="inline-code">このサイトの生い立ち</span>&nbsp;<span className="inline-code">React</span>&nbsp;<span className="inline-code">Ruby</span>&nbsp;<span className="inline-code">TypeScript</span>&nbsp;<span className="inline-code">設計</span><br />
...
Markdown&nbsp;形式で書いた記事のタイトルをカレンダーに表示したいと思ったがカレンダー部分は&nbsp;tsx&nbsp;ゴリゴリで今後も柔軟な変更が求められそうだったので以下のように対策することにした<br />

&nbsp;<br />

1.&nbsp;article.md&nbsp;の1行目を読み取って&nbsp;ruby&nbsp;スクリプトで&nbsp;json&nbsp;に吐き出す<br />

2.&nbsp;吐き出した&nbsp;json&nbsp;を&nbsp;Typescript&nbsp;風に読み込んで&nbsp;ReactDOM&nbsp;で表示する<br />

&nbsp;<br />

<h2>結果</h2>

背景に書いた通りのプロセスで実現した<br />

&nbsp;<br />

<a href={"https://github.com/shimomuh/shimomuh.github.io/commit/23fec03e5f7656ccf9dab360d7d4fe7fd1ec460a"}>→成果</a><br />

&nbsp;<br />

今回はその中でも躓いた部分について解説する<br />

<h2>過程</h2>

<h3>Typescript&nbsp;で&nbsp;json&nbsp;ってどう読むの？</h3>

非同期の方法は色々あるし知っているけど、そうじゃなくて簡素に&nbsp;json&nbsp;を読みたいんだ！<br />

ファイルサイズが増えてきたら&nbsp;<span className="inline-code">saga.js</span>&nbsp;とか使って対策するからまずシンプルな方法を教えてくれ！<br />

&nbsp;<br />

めっちゃ簡単でした<br />

<a href={"https://stackoverflow.com/questions/49996456/importing-json-file-in-typescript"}>Importing&nbsp;json&nbsp;file&nbsp;in&nbsp;TypeScript&nbsp;-&nbsp;stack&nbsp;overflow</a><br />
...
案の定、型を定義してないから定義してくれよ！って怒られる<br />

&nbsp;<br />

<span className="inline-code">Index&nbsp;signature&nbsp;of&nbsp;object&nbsp;type&nbsp;implicitly&nbsp;has&nbsp;an&nbsp;&#39;any&#39;&nbsp;type.</span><br />

&nbsp;<br />

そこで&nbsp;<a href={"https://qiita.com/gonta/items/fb7b9e6d0f12060c27d6"}>Typescriptで、Objectkeyとすると出るIndex&nbsp;signature&nbsp;of&nbsp;object&nbsp;type&nbsp;implicitly&nbsp;has&nbsp;an&nbsp;&#39;any&#39;&nbsp;type.を正しく回避する</a>&nbsp;に書いてあったことを参考に冗長に対応した<br />

<i>多分もっとうまい方法はある</i><br />
...
今回&nbsp;ruby&nbsp;スクリプトで&nbsp;json&nbsp;ファイルに書き出したときの方法は以下の通り<br />

成果だけだと見辛そうだったので完結に書いてみた<br />
...
いい加減リファクタしたい欲がすごいけど、多分明日はタグをリンクにする作業をやる<br />
