...
レスポンシブデザインで生きたい人生だった（端末サイズなどに依存せず実装できるのは素晴らしい！）<br />

が、カレンダーは縮小すると流石に可読性が落ちる<br />

&nbsp;<br />

そこで、レスポンシブを取り入れつつ、メディアクエリで可読性が下がると感じた画面サイズからデザインを変更する方法をとる<br />

このデザイン設計のメリットは以下の通り<br />

<ul><li>可読性が落ちない</li>
<li>スマホかどうかを&nbsp;UA&nbsp;なんかで判断すると、特定端末がうまく拾えなくて死ぬ、みたいなケースを回避できる</li>
</ul>
特に後者は悩みもので、昔&nbsp;Ruby&nbsp;on&nbsp;Rails&nbsp;の&nbsp;<a href={"https://github.com/jpmobile/jpmobile"}>jpmobile</a>&nbsp;という&nbsp;Gem&nbsp;を使って端末をサーバサイドで判断して出力先テンプレートを変えるということをしていたが、これだと後者の問題にひっかかる<br />

&nbsp;<br />

とはいえ、無理にメディアクエリを濫用すると、&nbsp;DOM&nbsp;構造が崩れたり&nbsp;<span className="inline-code">display:&nbsp;none;</span>&nbsp;でも&nbsp;html&nbsp;として出力してはいけないような機微情報やセキュリティ情報はサーバ側で&nbsp;render&nbsp;すら制約した方が無難なので使えない場面もある<br />

<h2>結果</h2>

今回は&nbsp;PC&nbsp;サイズをテーブルレイアウト、&nbsp;SP&nbsp;サイズをリストレイアウトに見せることで実現した<br />

&nbsp;<br />

<a href={"https://github.com/shimomuh/shimomuh.github.io/commit/842e8a628fc405d61bd6f2a6f7ee97098e8c95a7"}>→成果</a><br />

<img src="/diary/2019-07-08/responsive-plus-media-query.gif" alt="" /><br />

<h2>過程</h2>

まずはテーブルレイアウトをリストレイアウトに見せるために&nbsp;DOM&nbsp;構造をいじってやる必要があった<br />

今回は&nbsp;flex&nbsp;を上手いこと使って実現した<br />
...
<span className='code__with-order'>&nbsp;&nbsp;ul&nbsp;&#123;&nbsp;list-style:&nbsp;none;&nbsp;&#125;&nbsp;/*&nbsp;テーブルレイアウト時のヘッダーにあたる曜日は隠す&nbsp;*/</span><br />
...
あたりを参照するのがいいかな&nbsp;<s>（一部書き方忘れてた）</s><br />
...
これで&nbsp;100%&nbsp;を超える&nbsp;8&nbsp;要素目からいい感じに折り返してくれる<br />
      <br />
      <div>
        <Link to='/'>戻る</Link>
      </div>
